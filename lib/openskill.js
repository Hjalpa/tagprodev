			...
			async function getAverageFromScore(score, gameid, duration) {
				return await db.select(`
					...
					FROM (
						...
						where
							tp_game.mapid = (SELECT mapid FROM tp_game WHERE id = $3)
							AND openskill IS NOT NULL
							AND tp_player.tpid IS NOT NULL
							AND tp_playergame.duration <= $2
						order by ABS(score - $1), ABS(tp_playergame.duration - $2)
						LIMIT 10
					) as subquery
				`, [score, duration, gameid], 'row')
			}
		}
