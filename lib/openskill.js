		...
		try {
			...
			for await (const game of games) {
				...
				for(const player of playerData) {
					...
					const someballs = [8, 50, 18, 12, 27, 22, 3, 2, 19, 46, 11, 129, 512, 469, 1345, 874, 1731, 2491, 4181, 3841, 2496, 4305, 2490]
					if (someballs.includes(player.playerid)) {
						const { mu, sigma } = rating()
						player.mu = mu
						player.sigma = sigma * 2
					}
				}
				...
				for(let player of result) {
					...
					// if some ball force their mu and sigma to default for save
					const someballs = [8, 50, 18, 12, 27, 22, 3, 2, 19, 46, 11, 129, 512, 469, 1345, 874, 1731, 2491, 4181, 3841, 2496, 4305, 2490]
					if (someballs.includes(player.playerid)) {
						const { mu, sigma } = rating()
						player.mu = mu
