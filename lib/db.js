...
const db = {
	...
	delete: (table, data=[]) => {
		...
		return pool.query(sql, query.data)
			.then(function(rtn) {
	...
	insert: (table, data=[], returns = true) => {
		if(table == null || data == null)
			throw 'bad params'
		...
		let sql = `INSERT INTO ${table} (${query.columns.join(',')}) VALUES (${query.clause.join(',')}) ${(returns === true ? 'RETURNING LASTVAL()' : '')}`
		return pool.query(sql, query.data)
			.then(rtn => {
				if(returns)
			...
			.catch(e => {db.error = e; console.log(e)})
	...
	insertUpdate: (table, data=[], conflict=[]) => {
		if(table == null || data == null)
			throw 'bad params'
