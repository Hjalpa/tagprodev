			...
			case 'eltp':
				switch(pos) {
					case 'd':
						return `
							Round(
								(prevent::decimal / max(prevent::decimal) over () * 3.5)::decimal +
								(key_return::decimal / max(key_return::decimal) over () * 1.5)::decimal +
								(reset_from_my_prevent::decimal / max(reset_from_my_prevent::decimal) over () * 1.25)::decimal +
								(reset_from_my_return::decimal / max(reset_from_my_return::decimal) over () * 1.25)::decimal +
					...
					case 'o':
						return `
							Round(
								...
								(COALESCE(cap::decimal, 0) / NULLIF(max(cap::decimal) over (), 0) * 5)::decimal +
								...
								(COALESCE(good_kiss::decimal, 0) / NULLIF(max(good_kiss::decimal) over (), 0) * 0.5)::decimal +
								(
									(
										CASE
