...
module.exports.init = async (req, res) => {
	try {
		...
		res.json({
			all: await getAllGames(),
			upsets: await getUpsets(),
			mine: await getMyGames(profileID, req),
		})
	} catch(e) {
...
async function getMyGames(tpid, req) {
...
async function logIP(profileID, ip) {
	...
	if(playerID)
		await db.insertUpdate('tp_playerip', {playerid: playerID, ip: ip}, ['playerid','ip'])
	else
