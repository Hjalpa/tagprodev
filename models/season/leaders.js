...
let init = async (req, res) => {
	try {
		...
		let data = {
			config: {
				title: req.mode.toUpperCase() + tier + ' Season ' + req.season + ' Leaders / ' + filters.mode.name,
				name: req.seasonname,
				path: req.baseUrl,
				season: req.season,
				tier: req.seasonTier,
...
async function getData(filters, sql) {
	...
	else if(filters.mode.get === 'gamepercent')
		percentage = " || '%' ";
			...
			return {
				...
				assists: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(assist)::DECIMAL / sum(assist_team_for + assist_team_against)::DECIMAL) * 100, 0)',
				...
				timeleading: {
					title: 'Time Leading',
					data: await getData(filters, {
						sum: `TO_CHAR(sum(position_win_time) * interval '1 sec', 'hh24:mi:ss')`,
						avg: `TO_CHAR(avg(position_win_time) * interval '1 sec', 'hh24:mi:ss')`,
				...
				pups: {
					title: 'Pups',
					data: await getData(filters, {
						sum: 'sum(pup_tp + pup_rb + pup_jj)',
						avg: 'ROUND(avg(pup_tp + pup_rb + pup_jj), 2)',
						teampercent: `
							ROUND(
								(
									sum(pup_tp + pup_rb + pup_jj)::DECIMAL
									/
									sum(pup_tp_team_for + pup_rb_team_for + pup_jj_team_for)::DECIMAL
								)
							* 100, 0)`,
						gamepercent: `
							ROUND(
								(
									sum(pup_tp + pup_rb + pup_jj)::DECIMAL
									/
									sum(pup_tp_team_for + pup_rb_team_for + pup_jj_team_for + pup_tp_team_against + pup_rb_team_against + pup_jj_team_against)::DECIMAL
								)
							* 100, 0)`,
						...
						gamepercent: `
							ROUND(
								(
									sum(pup_tp)::DECIMAL
									/
									sum(pup_tp_team_for + pup_tp_team_against)::DECIMAL
								)
							* 100, 0)`,
				...
				tags: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(tag)::DECIMAL / sum(tag_team_for + tag_team_against)::DECIMAL) * 100, 0)',
				...
				nonreturntags: {
					...
					data: await getData(filters, {
						sum: 'sum(tag - return)',
						avg: 'ROUND(avg(tag - return), 2)',
						teampercent: `
							ROUND(
								(
									sum(tag - return)::DECIMAL
									/
									sum(tag_team_for - return_team_for)::DECIMAL
								)
							* 100, 0)`,
						gamepercent: `
							ROUND(
								(
									sum(tag - return)::DECIMAL
									/
									sum(tag_team_for - return_team_for + tag_team_against - return_team_against)::DECIMAL
								)
							* 100, 0)`,
				...
				returnswithinownhalf: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(return_within_my_half)::DECIMAL / sum(return_within_my_half_team_for + return_within_my_half_team_against)::DECIMAL) * 100, 0)',
				...
				returnswithinopphalf: {
					...
					data: await getData(filters, {
						sum: 'sum(return - return_within_my_half)',
						avg: 'ROUND(avg(return) - avg(return_within_my_half), 2)',
						teampercent: 'ROUND((sum(return - return_within_my_half)::DECIMAL / sum(return_team_for - return_within_my_half_team_for)::DECIMAL) * 100, 0)',
						gamepercent: 'ROUND((sum(return - return_within_my_half)::DECIMAL / sum(return_team_for - return_within_my_half_team_for + return_team_against - return_within_my_half_team_against)::DECIMAL) * 100, 0)',
				...
				teamholdwilstprevent: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(prevent_whilst_team_hold_time)::DECIMAL / sum(hold_whilst_prevent_team_for + hold_whilst_prevent_team_against)::DECIMAL) * 100, 0)',
				...
				holdwhilstteamprevent: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(hold_whilst_team_prevent_time)::DECIMAL / sum(hold_whilst_prevent_team_for + hold_whilst_prevent_team_against)::DECIMAL) * 100, 0)',
				...
				holdwhilstopponentsdont: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(hold_whilst_opponents_dont)::DECIMAL / sum(hold_whilst_opponents_dont_team_for + hold_whilst_opponents_dont_team_against)::DECIMAL) * 100, 0)',
				...
				grabs: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(grab)::DECIMAL / sum(grab_team_for + grab_team_against)::DECIMAL) * 100, 0)',
				...
				grabwhilstopponentsprevent: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(grab_whilst_opponents_prevent)::DECIMAL / sum(grab_whilst_opponents_prevent_team_for + grab_whilst_opponents_prevent_team_against)::DECIMAL) * 100, 0)',
				...
				grabwhilstopponentshold: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(grab_whilst_opponents_hold)::DECIMAL / sum(grab_whilst_opponents_hold_team_for + grab_whilst_opponents_hold_team_against)::DECIMAL) * 100, 0)',
				...
				failedcaps: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(drop_within_2_tiles_from_my_base)::DECIMAL / sum(return_within_2_tiles_from_opponents_base_team_against + return_within_2_tiles_from_opponents_base_team_for)::DECIMAL) * 100, 0)',
				...
				quickreturn: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(quick_return)::DECIMAL / sum(quick_return_team_for + quick_return_team_against)::DECIMAL) * 100, 0)',
				...
				keyreturns: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(key_return)::DECIMAL / sum(key_return_team_for + key_return_team_against)::DECIMAL) * 100, 0)',
				...
				saves: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(return_within_2_tiles_from_opponents_base)::DECIMAL / sum(return_within_2_tiles_from_opponents_base_team_for + return_within_2_tiles_from_opponents_base_team_against)::DECIMAL) * 100, 0)',
				...
				clearances: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(return_within_5_tiles_from_opponents_base)::DECIMAL / sum(return_within_5_tiles_from_opponents_base_team_for + return_within_5_tiles_from_opponents_base_team_against)::DECIMAL) * 100, 0)',
				...
				resets: {
					title: 'Resets',
					data: await getData(filters, {
						sum: 'sum(reset_from_my_prevent + reset_from_my_return)',
						avg: 'ROUND(avg(reset_from_my_prevent + reset_from_my_return), 2)',
						teampercent: `
							ROUND(
								(
									sum(reset_from_my_prevent + reset_from_my_return)::DECIMAL
									/
									sum(reset_team_for)::DECIMAL
								)
							* 100, 0)`,
						gamepercent: `
							ROUND(
								(
									sum(reset_from_my_prevent + reset_from_my_return)::DECIMAL
									/
									sum(reset_team_for + reset_team_against)::DECIMAL
								)
							* 100, 0)`,
				...
				handoffs: {
					title: 'Handoffs',
					data: await getData(filters, {
						sum: 'sum(handoff_drop + handoff_pickup)',
						avg: 'ROUND(avg(handoff_drop + handoff_pickup), 2)',
						teampercent: `
							ROUND(
								(
									sum(handoff_drop + handoff_pickup)::DECIMAL
									/
									sum(handoff_team_for)::DECIMAL
								)
							* 100, 0)`,
						gamepercent: `
							ROUND(
								(
									sum(handoff_drop + handoff_pickup)::DECIMAL
									/
									sum(handoff_team_for + handoff_team_against)::DECIMAL
								)
							* 100, 0)`,
				...
				kisses: {
					...
					data: await getData(filters, {
						...
						teampercent: 'ROUND((sum(kiss)::DECIMAL / sum(kiss_team)::DECIMAL) * 100, 0)',
						gamepercent: 'ROUND((sum(kiss)::DECIMAL / sum(kiss_team)::DECIMAL) * 100, 0)',
				...
				nondroppops: {
					...
					data: await getData({...filters, ...{having: true, ascending: true}}, {
						sum: 'sum(pop - drop)',
						avg: 'ROUND(avg(pop - drop), 2)',
						teampercent: `
							ROUND(
								(
									sum(pop - drop)::DECIMAL
									/
									sum(pop_team_for - drop_team_for)::DECIMAL
								)
							* 100, 0)`,
						gamepercent: `
							ROUND(
								(
									sum(pop - drop)::DECIMAL
									/
									sum(pop_team_for - drop_team_for + pop_team_against - drop_team_against)::DECIMAL
								)
							* 100, 0)`,
				...
				tagpop: {
					...
					data: await getData({...filters, ...{ascending: false}}, {
						sum: 'sum(tag - pop)',
						avg: 'ROUND(avg(tag - pop), 2)',
						teampercent: `
							ROUND(
								(
									sum(tag - pop)
									/
									sum(tag_team_for - pop_team_for)
								)
								* 100
							, 0)
						`,
						gamepercent: `
							ROUND(
								(
									sum(tag - pop)::DECIMAL
									/
									sum((tag_team_for + tag_team_against) - (pop_team_for + pop_team_against))::DECIMAL
								)
								* 100
							, 0)
						`,
				...
				capfrompup: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(cap_whilst_team_have_active_pup)::DECIMAL / sum(cap_whilst_team_have_active_pup_team_for + cap_whilst_team_have_active_pup_team_against)::DECIMAL) * 100, 0)',
				...
				capfrommyregrab: {
					...
					data: await getData(filters, {
						...
						teampercent: 'ROUND((sum(cap_from_my_regrab)::DECIMAL / sum(cap_from_regrab_team_for)::DECIMAL) * 100, 0)',
						gamepercent: 'ROUND((sum(cap_from_my_regrab)::DECIMAL / sum(cap_from_regrab_team_for + cap_from_regrab_team_against)::DECIMAL) * 100, 0)',
				...
				capfromprevent: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(cap_from_prevent)::DECIMAL / sum(cap_from_prevent_team_for + cap_from_prevent_team_against)::DECIMAL) * 100, 0)',
				...
				assistfromprevent: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(cap_from_my_prevent)::DECIMAL / sum(cap_from_prevent_team_for + cap_from_prevent_team_against)::DECIMAL) * 100, 0)',
				...
				capsfromroughgrabs: {
					title: 'Caps from Rough Grabs',
					data: await getData({...filters, ...{having: true}}, {
						sum: 'sum(cap_from_grab_whilst_opponents_prevent)',
						avg: 'ROUND(avg(cap_from_grab_whilst_opponents_prevent), 2)',
						teampercent: `CASE WHEN sum(cap_from_grab_whilst_opponents_prevent) = 0 THEN 0 ELSE ROUND((sum(cap_from_grab_whilst_opponents_prevent)::DECIMAL / sum(cap_from_grab_whilst_opponents_prevent_team_for)::DECIMAL) * 100, 0) END`,
						gamepercent: `CASE WHEN sum(cap_from_grab_whilst_opponents_prevent) = 0 THEN 0 ELSE ROUND((sum(cap_from_grab_whilst_opponents_prevent)::DECIMAL / sum(cap_from_grab_whilst_opponents_prevent_team_for + cap_from_grab_whilst_opponents_prevent_team_against)::DECIMAL) * 100, 0) END`,
						// top: 'playergame.cap_from_grab_whilst_opponents_prevent = (SELECT cap_from_grab_whilst_opponents_prevent',
					}),
				},
				longhold: {
					title: 'Long Holds',
						    ...
						    ROUND(
								(
									COALESCE(
										NULLIF(sum(long_hold)::DECIMAL, 0)
										/
										NULLIF(sum(long_hold_team_for + long_hold_team_against)::DECIMAL, 0)
									, 0)
								* 100)
							, 0)
						`,
				...
				longholdandcap: {
					title: 'Caps from Long Hold',
					data: await getData({...filters, ...{having: true}}, {
						sum: 'sum(long_hold_and_cap)',
						avg: 'ROUND(avg(long_hold_and_cap), 2)',
						teampercent: 'ROUND((sum(long_hold_and_cap)::DECIMAL / sum(long_hold_and_cap_team_for)::DECIMAL) * 100, 0)',
						gamepercent: 'ROUND((sum(long_hold_and_cap)::DECIMAL / sum(long_hold_and_cap_team_for + long_hold_and_cap_team_against)::DECIMAL) * 100, 0)',
						// top: 'playergame.long_hold_and_cap = (SELECT long_hold_and_cap',
					}),
				},
				buttoning: {
					title: 'Buttoning',
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(button)::DECIMAL / sum(button_team_for + button_team_against)::DECIMAL) * 100, 0)',
			...
			return {
				...
				block: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(block)::DECIMAL / sum(block_team_for + block_team_against)::DECIMAL) * 100, 0)',
				...
				takeovers: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(takeover)::DECIMAL / sum(takeover_team_for + takeover_team_against)::DECIMAL) * 100, 0)',
				...
				dispossessed: {
					...
					data: await getData(filters, {
						sum: 'sum(takeover_good - dispossessed)',
						avg: 'ROUND(avg(takeover_good - dispossessed), 2)',
						teampercent: `
							ROUND(
								(
									sum(takeover_good - dispossessed)::DECIMAL
									/
									sum(takeover_good_team_for - dispossessed_team_for)::DECIMAL
								)
								* 100
							, 0)
						`,
						gamepercent: `
							ROUND(
								(
									sum(takeover_good - dispossessed)::DECIMAL
									/
									sum((takeover_good_team_for + takeover_good_team_against) - (dispossessed_team_for + dispossessed_team_against))::DECIMAL
								)
								* 100
							, 0)
						`,
				...
				strongtakeovers: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(takeover_good)::DECIMAL / sum(takeover_good_team_for + takeover_good_team_against)::DECIMAL) * 100, 0)',
				...
				regrabs: {
					title: 'Regrabs',
					data: await getData(filters, {
						sum: 'sum(grab - takeover)',
						avg: 'ROUND(avg(grab - takeover), 2)',
						teampercent: `
							ROUND(
								(
									sum(grab - takeover)::DECIMAL
									/
									sum(grab_team_for - takeover_team_for)::DECIMAL
								)
							* 100, 0)`,
						gamepercent: `
							ROUND(
								(
									sum(grab - takeover)::DECIMAL
									/
									sum((grab_team_for + grab_team_against) - (takeover_team_for + takeover_team_against))::DECIMAL
								)
							* 100, 0)`,
				...
				chains: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(chain)::DECIMAL / sum(chain_team_for + chain_team_against)::DECIMAL) * 100, 0)',
				...
				tapinfromychain: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(tapin_from_my_chain)::DECIMAL / sum(tapin_from_my_chain_team_for + tapin_from_my_chain_team_against)::DECIMAL) * 100, 0)',
				...
				tapincaps: {
					...
					data: await getData(filters, {
						...
						gamepercent: 'ROUND((sum(cap_from_tapin)::DECIMAL / sum(cap_from_tapin_team_for + cap_from_tapin_team_against)::DECIMAL) * 100, 0)',
						...
						gamepercent: `
							ROUND(
								(
									sum(tag - return)::DECIMAL
									/
									sum((tag_team_for + tag_team_against) - (return_team_for - return_team_against))::DECIMAL
								)
							* 100, 0)`,
						top: '(playergame.tag - playergame.return) = (SELECT (tag - return)',
					}),
