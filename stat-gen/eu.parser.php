...
class euParser {
	...
	public $cap_from_tapin;
	public $tapin_from_my_chain;
	public $dispossessed;
...
# --------------------------------------------------------------------------
	...
	public function active($player, $team, $time) {
		// overwrite team (this fixes the team=0 bug, where if a player joins after the game starts. i.e. see euid: 2928334)
...
# --------------------------------------------------------------------------
	...
	public function hold_stop($player, $time, $events = false, $pop = true, $cap = false) {
		...
		$this->takeover($player, $time);
		$this->takeover_good($player, $time);
		$this->dispossessed($player, $time);
		$this->chain($player, $time);
...
# --------------------------------------------------------------------------
	...
	public function cap($player, $time) {
		...
		$this->cap_from_tapin($player, $time);
...
# --------------------------------------------------------------------------
	...
	public function drop($player, $time, $events) {
		...
		$this->handoff($player, $time);
		$this->dispossessed_start($player, $time, $events);
...
# --------------------------------------------------------------------------
	...
	public function cap_from_tapin($player, $time) {
		$hold_start = $this->tmp['hold_start'][$player];
		$hold = ($time - $hold_start);
		$hold_seconds = $this->seconds($hold);

		if($hold_seconds <= 1) {
			$this->tapin_from_my_chain($player, $time);

			$this->cap_from_tapin['player'][$player]++;

			foreach($this->active as $p => $pt)
				if($pt == $this->active[$player])
					$this->cap_from_tapin['team']['for'][$p]++;
				else
					$this->cap_from_tapin['team']['against'][$p]++;
		}

		$this->takeover_good_tapin_check($player, $time);
	}

# 	tapin from my chain
# --------------------------------------------------------------------------

	public function tapin_from_my_chain($player, $time) {
		$team = $this->getTeam($player);

		foreach($this->tmp['hold'][$team] as $p => $pt) {
			...
			if($p != $player) {
				...
				if($hold_seconds <= 2) {
					$this->tapin_from_my_chain['player'][$p]++;

					foreach($this->active as $_p => $_pt)
						if($_pt == $this->active[$p])
							$this->tapin_from_my_chain['team']['for'][$_p]++;
						else
							$this->tapin_from_my_chain['team']['against'][$_p]++;
...
# --------------------------------------------------------------------------
	...
	public function cap_whilst_having_active_pup($player, $time) {
		...
		// grabbed pup AFTER hold and hasn't dropped yet
		if($this->tmp['pup_expired']['player'][$player] >= $this->tmp['hold_start'][$player] || isset($this->tmp['pup_active']['player'][$player]) || $expiredAgo < 5) {
			$this->cap_whilst_having_active_pup['player'][$player]++;

			foreach($this->active as $_p => $_pt)
				if($_pt == $this->active[$player])
					$this->cap_whilst_having_active_pup['team']['for'][$_p]++;
				else
					$this->cap_whilst_having_active_pup['team']['against'][$_p]++;
		}

		// check if team have a pup expire whilst I hold
		$check_team_pup_active = false;
		foreach($this->tmp['pup_expired']['player'] as $p => $pt) {
			if($this->getTeam($p) != $this->getTeam($player)) {
				if($this->tmp['pup_expired']['player'][$p] >= $this->tmp['hold_start'][$player]) {
					$check_team_pup_active = true;
				}
			}
		}

		if($check_team_pup_active || isset($this->tmp['pup_active']['team'][$teamID]) || $expiredAgoTeam < 5) {
			$this->cap_whilst_team_have_active_pup['player'][$player]++;

			foreach($this->active as $_p => $_pt)
				if($_pt == $this->active[$player])
					$this->cap_whilst_team_have_active_pup['team']['for'][$_p]++;
				else
					$this->cap_whilst_team_have_active_pup['team']['against'][$_p]++;
		}
	}
	...
	public function assist($p) {
		foreach($this->tmp['assist'] as $player => $v) {
			...
			if($check_assist) {
				foreach($this->active as $_p => $_pt) {
					if($_pt == $this->active[$p])
						$this->assist['team']['for'][$_p]++;
					else
						$this->assist['team']['against'][$_p]++;
				}
			}
		}
...
# --------------------------------------------------------------------------
	...
	// check length between each return is <= 1 second

	public function chain($player, $time) {
		$hold_start = $this->tmp['hold_start'][$player];

		foreach($this->tmp['dispossessed_start'] as $p => $pt) {
			if($hold_start === $pt['time']) {

				$chain_player = $this->tmp['dispossessed_history']['original_hold'];
				$returned_by = $this->tmp['dispossessed_history']['returned_by'];
				$returned_at = $this->tmp['dispossessed_history']['returned_at'];

				// check drop + pickup players aren't from same team
				if($this->active[$chain_player] === $this->active[$p]) continue;

				if($this->seconds($returned_at - $hold_start) <= 1) {
					// echo '-------------------------' . "\n";
					// echo $p . ' chains:' . "\n";
					// echo $chain_player . ' returns ' . $p . ' at ' . $this->_sec($hold_start) . "\n";
					// echo $returned_by . ' returns ' . $chain_player . ' at ' . $this->_sec($returned_at) . "\n";
					// echo $this->seconds($returned_at - $hold_start)  . "\n";

					$this->chain['player'][$p]++;

					foreach($this->active as $_p => $_pt) {
						if($_pt == $this->active[$p])
							$this->chain['team']['for'][$_p]++;
						else
							$this->chain['team']['against'][$_p]++;
					}
				}
			}
		}
...
# --------------------------------------------------------------------------
	...
	public function takeover($player, $time) {
		...
		foreach($this->tmp['dispossessed_start'] as $p => $pt) {
			if($hold_start === $pt['time']) {
				// if($pt['returned_by'] != 'nom') continue;
				// echo $player . "\n";
				// echo $p . ' returned by  ' . $pt['returned_by'] . " at ". $this->_sec($pt['time']) . "\n";
				$this->takeover['player'][$pt['returned_by']]++;

				foreach($this->active as $_p => $_pt)
					if($_pt == $this->active[$player])
						$this->takeover['team']['for'][$_p]++;
					else
						$this->takeover['team']['against'][$_p]++;
			}
		}
	...
	// detailed takeover
	// requires 3 seconds or greater than hold AFTER the return
	public function takeover_good($player, $time) {
		...
		if($hold_seconds >= 3) {
			// echo '-------------------------' . "\n";
			// echo $player . ' held for ' . $this->_sec($hold) . ' at ' . $this->_sec($time)  .  "\n";

			foreach($this->tmp['dispossessed_start'] as $p => $pt) {
				if($hold_start === $pt['time'] && $pt['returned_by'] === $player) {

					$this->takeover_good['player'][$player]++;

					foreach($this->active as $_p => $_pt)
						if($_pt == $this->active[$player])
							$this->takeover_good['team']['for'][$_p]++;
						else
							$this->takeover_good['team']['against'][$_p]++;
				}
			}
		}
	}

	// this increments if the takeover_good is a tapin (< 3 seconds hold)
	// without this check, any tapins won't count as a takeover_good.
	// DO I ACTUALLY WANT THIS?
	public function takeover_good_tapin_check($player, $time) {
		...
		if($hold_seconds < 3) {
			...
			foreach($this->active as $p => $pt)
				if($pt == $this->active[$player])
					$this->takeover_good['team']['for'][$p]++;
				else
					$this->takeover_good['team']['against'][$p]++;
		}
	}
...
# --------------------------------------------------------------------------
	...
	public function dispossessed_start($player, $time, $events) {
		foreach($events as $tE) {
			$_player = explode('## ', $tE)[0];
			$_msg = explode('## ', $tE)[1];
			...
			if($player === $_player)
				...
				$this->tmp['dispossessed_start'][$player] = [
					'time' => $time,
					'returned_by' => $_player,
				];
			}
	...
	// if dropped flag and returnee holds for longer than 3 seconds
	public function dispossessed($player, $time) {
		...
		foreach($this->tmp['dispossessed_start'] as $p => $pt) {
			if($time === $pt['time']) {
				...
				if($hold_seconds >= 3) {
					$original_player = $this->tmp['dispossessed_history']['original_hold'];

					// check drop + pickup players aren't from same team
					if($this->active[$player] === $this->active[$original_player]) continue;

					// echo '-------------------------' . "\n";
					// echo $player . ' dispossesses ' . $original_player . "\n";
					// echo $pt['returned_by'] . ' returns ' . $player . ' at ' . $this->_sec($pt['time']) . ' - ' . $player . ' held for ' . $hold_seconds. "\n";

					$this->dispossessed['player'][$original_player]++;

					foreach($this->active as $_p => $_pt)
						if($_pt == $this->active[$original_player])
							$this->dispossessed['team']['for'][$_p]++;
						else
							$this->dispossessed['team']['against'][$_p]++;
				}

				// history
				$this->tmp['dispossessed_history'] = [
					'original_hold' => $player,
					'original_hold_length' => $hold_seconds,
					'returned_by' => $pt['returned_by'],
					'returned_at' => $pt['time'],
				];
			}
		}
...
# --------------------------------------------------------------------------
	...
	public function seconds($time) {
		// return (int)round(($time / 3600) * 60, 0);
		return (($time / 3600) * 60);
	}
