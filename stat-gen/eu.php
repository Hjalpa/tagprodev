...
class eu {
	...
	private function match($extras) {
		...
		$matchdata = [
			...
			'datetime' => gmdate('Y-m-d H:i:s', $data['date']),
			'server' => $data['server'],
			'timelimit' => $data['timeLimit'],
			'duration' => round($data['duration'] / 60),
			// 'duration_time' => gmdate('H:i:s', ($data['duration'] / 60)),
		...
		foreach($data['players'] as $p) {
			...
			// bug here that sets team=0 for players that join after game starts. fix at parser.php:77
		...
		return [
			'game' => $matchdata,
			'players' => $this->format($store, $extras),
		];
	}
	...
	private function format($data, $extras) {
		foreach($data->score['player'] as $p => $arr) {
			...
			$obj[] = [
                ...
                'play_time' => round($data->seconds($data->play_minutes['player'][$p])),
				...
				'assist_team_for' => $data->assist['team']['for'][$p] | 0,
				'assist_team_against' => $data->assist['team']['against'][$p] | 0,

				'cap_whilst_having_active_pup' => $data->cap_whilst_having_active_pup['player'][$p] | 0,
				'cap_whilst_having_active_pup_team_for' => $data->cap_whilst_having_active_pup['team']['for'][$p] | 0,
				'cap_whilst_having_active_pup_team_against' => $data->cap_whilst_having_active_pup['team']['against'][$p] | 0,

				'cap_whilst_team_have_active_pup' => $data->cap_whilst_team_have_active_pup['player'][$p] | 0,
				'cap_whilst_team_have_active_pup_team_for' => $data->cap_whilst_team_have_active_pup['team']['for'][$p] | 0,
				'cap_whilst_team_have_active_pup_team_against' => $data->cap_whilst_team_have_active_pup['team']['against'][$p] | 0,
				...
				// nf specifics
                'cap_from_tapin' => $data->cap_from_tapin['player'][$p] | 0,
                'cap_from_tapin_team_for' => $data->cap_from_tapin['team']['for'][$p] | 0,
                'cap_from_tapin_team_against' => $data->cap_from_tapin['team']['against'][$p] | 0,

				'tapin_from_my_chain' => $data->tapin_from_my_chain['player'][$p] | 0,
                'tapin_from_my_chain_team_for' => $data->tapin_from_my_chain['team']['for'][$p] | 0,
                'tapin_from_my_chain_team_against' => $data->tapin_from_my_chain['team']['against'][$p] | 0,

				// drop + team won't win flag back within 3 seconds, thus creating a situation where you don't chain or put your team in a position to win the flag
				'dispossessed' => $data->dispossessed['player'][$p] | 0,
				'dispossessed_team_for' => $data->dispossessed['team']['for'][$p] | 0,
				'dispossessed_team_against' => $data->dispossessed['team']['against'][$p] | 0,

				// return-grab + hold for more than 3 seconds
				'takeover' => $data->takeover['player'][$p] | 0,
				'takeover_team_for' => $data->takeover['team']['for'][$p] | 0,
				'takeover_team_against' => $data->takeover['team']['against'][$p] | 0,

				'takeover_good' => $data->takeover_good['player'][$p] | 0,
				'takeover_good_team_for' => $data->takeover_good['team']['for'][$p] | 0,
				'takeover_good_team_against' => $data->takeover_good['team']['against'][$p] | 0,

				'chain' => $data->chain['player'][$p] | 0,
				'chain_team_for' => $data->chain['team']['for'][$p] | 0,
				'chain_team_against' => $data->chain['team']['against'][$p] | 0,
			];
		}
